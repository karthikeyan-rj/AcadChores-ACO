  ...
  return (
    <div className="p-6 space-y-5 max-w-[1200px] mx-auto bg-app min-h-screen">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-theme">Workflows</h1>
          <p className="text-xs text-theme-tertiary mt-0.5">
            {filtered.length} workflows{runningCount > 0 ? <span className="text-status-active"> · {runningCount} running</span> : ''}
          </p>
        </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="relative">
          <button onClick={() => setSortBy(p => p === 'newest' ? 'oldest' : p === 'oldest' ? 'status' : 'newest')}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium bg-surface-2 border border-theme text-theme-tertiary hover:text-theme-secondary transition cursor-pointer">
            <ArrowUpDown size={12} />
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16">
          <GitBranch size={32} className="text-theme-tertiary mb-3" />
          <p className="text-sm font-medium text-theme-secondary">No workflows found</p>
          ...
          <AnimatePresence>
            {filtered.map((ex, i) => (
              <motion.div key={ex._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                ...
                className={cn(
                  'rounded-[14px] border transition p-4 cursor-pointer group relative',
                  isRunning(ex.status)
                    ? 'border-status-active bg-status-active-soft'
                    : 'border-theme bg-surface hover:bg-surface-hover'
                )}>
                <div className="flex items-start justify-between mb-2">
                  <span className={cn(
                    'text-[10px] font-semibold uppercase px-2 py-0.5 rounded',
                    statusBg(ex.status), statusColor(ex.status)
                  )}>{ex.status}</span>
                </div>
                <p className="text-sm font-medium truncate mb-1 text-theme">{ex.description || ex.title || `Workflow ${ex._id.slice(-6)}`}</p>
                <div className="flex items-center gap-3 text-[10px] text-theme-tertiary">
                  ...
                  {ex.completed_at && <span className="flex items-center gap-1 text-status-active">{formatDuration(ex.started_at, ex.completed_at)}</span>}
